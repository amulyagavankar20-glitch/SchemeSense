            ...
            <CardHeader>
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  >
                    {scheme.relevance || 95}% match
                  </Badge>
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className={`h-8 w-8 ${savedSchemeIds.includes(scheme.id) ? "text-primary fill-primary" : ""}`}
                    onClick={() => toggleBookmark(scheme.id)}
                  >
                    <Bookmark className={`h-4 w-4 ${savedSchemeIds.includes(scheme.id) ? "fill-current" : ""}`} />
                  </Button>
                </div>
              </div>
